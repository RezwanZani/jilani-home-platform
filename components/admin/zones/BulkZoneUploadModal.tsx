...
import { Upload, X, CheckCircle, FileSpreadsheet, Loader2, FileText } from "lucide-react";
import { bulkInsertZones } from "@/lib/actions/zone-actions";
...
interface BulkZoneUploadModalProps {
    isOpen: boolean;
    onClose: () => void;
    onComplete?: () => void;
}

export default function BulkZoneUploadModal({ isOpen, onClose, onComplete }: BulkZoneUploadModalProps) {
    ...
    const downloadTemplate = (format: 'csv' | 'xlsx') => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Zones_Template");
        XLSX.writeFile(workbook, `Jilani_Zones_Template.${format}`);
    };
    ...
    const confirmUpload = async () => {
        ...
        const result = await bulkInsertZones(fullData);

        if (result.success) {
            alert(`Successfully added ${result.count} zones!`);
            resetState();
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Bulk Upload Zones</h2>
                            </div>
                            <button
                                onClick={handleClose}
                            ...
                            <div className="space-y-4">
                                    ...
                                    <table className="w-full text-left text-sm whitespace-nowrap">
                                        <thead className="bg-gray-50 dark:bg-white/5 text-gray-600 dark:text-gray-400">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 font-semibold">Thana</th>
                                        ...
                                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                            {previewData.map((row, idx) => (
                                                <tr key={idx} className="text-gray-800 dark:text-gray-200">
                                                    ...
                                                    <td className="px-4 py-3">{row.name_bn || row['name_bn'] || <span className="text-red-500 font-bold">Missing</span>}</td>
                                                    <td className="px-4 py-3">{row.city || row.City || '-'}</td>
                                                    <td className="px-4 py-3">{row.thana || row.Thana || '-'}</td>
                                                    <td className="px-4 py-3">{row.area || row.area || '-'}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                ...
                                <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/5">
                                    ...
                                    <Button onClick={confirmUpload} disabled={isUploading} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold">
                                        ...
                                        {isUploading ? "Saving to Database..." : `Confirm & Save ${fullData.length} Zones`}
                                    </Button>
                                </div>
                            </div>
                        )}

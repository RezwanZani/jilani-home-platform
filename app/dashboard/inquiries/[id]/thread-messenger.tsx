...
import { sendTicketMessage, toggleTicketStatus } from "@/lib/actions/ticket-actions";
import { getPresignedR2Url } from "@/lib/actions/uploads";
import { toast } from "sonner";
import { Loader2, Send, Paperclip } from "lucide-react";
import { format } from "date-fns";

export function ThreadMessenger({ ticket, messages, currentUserId }: { ticket: any, messages: any[], currentUserId: string }) {
    ...
    const handleReopen = async () => {
        setLoading(true);
        const res = await toggleTicketStatus(ticket.id, 'open');
        if (res.success) {
